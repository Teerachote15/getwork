...
class PostJobHirePage extends StatefulWidget {
  const PostJobHirePage({super.key});

  @override
  State<PostJobHirePage> createState() => _PostJobHirePageState();
}

class _PostJobHirePageState extends State<PostJobHirePage> {
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  maxLines: 4,
                  decoration: InputDecoration(
                    hintText: 'อธิบายรายละเอียดงานที่คุณอยากได้\nบริฟงานให้ฟรีแลนซ์เข้าใจโดยละเอียด',
                    filled: true,
                    fillColor: Colors.white,
                    border: OutlineInputBorder(
                      borderRadius: BorderRadius.circular(16),
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: _sampleImage != null
                        ? ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.file(_sampleImage!, fit: BoxFit.cover, width: double.infinity))
                        : Center(child: Text('เลือกไฟล์รูป (แตะเพื่ออัปโหลด)', style: TextStyle(color: Colors.grey[700], fontSize: 15 * textScale))),
                  ),
                ),
                SizedBox(height: screenHeight * 0.04),
                // งบประมาณ/ค่าแรง
  ...
  Future<void> _submitPost() async {
    ...
    try {
      if (_sampleImage != null) {
        ...
        request.fields['post_type'] = 'employer'; // เพิ่ม post_type
        request.files.add(await http.MultipartFile.fromPath('image', _sampleImage!.path));
        final streamed = await request.send();
        final respStr = await streamed.stream.bytesToString();
        if (streamed.statusCode == 200 || streamed.statusCode == 201) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('โพสต์งานสำเร็จ')));
          ...
          if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('โพสต์ไม่สำเร็จ: $respStr')));
        }
      } else {
        ...
        final body = {
          ...
          'post_type': 'employer', // เพิ่ม post_type
        };
        ...
        if (res.statusCode == 200 || res.statusCode == 201) {
          ...
          if (mounted) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('โพสต์ไม่สำเร็จ: $msg')));
          }
        }
      }
    } catch (e) {
  ...
  Future<void> _loadCategories() async {
    try {
      ...
      // ignore errors silently; dropdown will be empty
    }
  }
